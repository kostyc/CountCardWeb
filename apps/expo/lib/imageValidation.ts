...
export const ROSTER_DOCUMENT_MAX_DIMENSION = 4096;
...
export async function validateRosterDocumentImage(input: {
  uri: string;
  mimeType?: string | null;
  fileSize?: number | null;
  width?: number;
  ...
  const width = input.width ?? 0;
  const height = input.height ?? 0;
  if (width > ROSTER_DOCUMENT_MAX_DIMENSION || height > ROSTER_DOCUMENT_MAX_DIMENSION) {
    return {
      ok: false,
      error: `Image dimensions must be ${ROSTER_DOCUMENT_MAX_DIMENSION}px or smaller on each side.`,
    };
  }
