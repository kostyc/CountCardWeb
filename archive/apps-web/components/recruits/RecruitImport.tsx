...
export function RecruitImport({ receivingMode = false, onImportComplete }: RecruitImportProps): JSX.Element {
  const { user } = useAuth();
  const { showToast } = useToast();
  const { canCreateAny, getOrganizationalScope } = useRecruitPermissions();
  const { canReceivingWorkflow, isFullAdmin } = useAdminAccess();
  const canImport = receivingMode ? canReceivingWorkflow || isFullAdmin : canCreateAny;
  ...
  useEffect(() => {
    if (!receivingMode) return;
    setOrgDefaults((prev) => ({
      regiment: prev.regiment ?? 'West',
  ...
  }, [getOrganizationalScope, receivingMode]);
      ...
      <Card className="p-6 space-y-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {receivingMode
              ? 'Import recruits into Receiving custody (Support/Receiving). Review rows and run a dry run before committing.'
        ...
        {receivingMode ? (
          <p className="text-sm text-gray-700 dark:text-gray-300">
            Org locked to Support Battalion / Receiving Company (platoon {RECEIVING_PLATOON}). Recruits enter{' '}
            <strong>receiving</strong> custody with default medical checklist.
          </p>
        ) : (
          <>
            {platoonHint && (
