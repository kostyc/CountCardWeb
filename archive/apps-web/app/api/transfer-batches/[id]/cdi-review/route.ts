/**
 * POST /api/transfer-batches/[id]/cdi-review — CDI staged review
 */

import { NextRequest, NextResponse } from 'next/server';
import { verifyAuthToken } from '@/lib/permissions/server';
import { advanceCdiReviewAdmin, getTransferBatchAdmin } from '@/lib/lifecycle/transferBatchAdmin';
  ...
  try {
    ...
    if (!canAdvanceTransferBatchStage(token, batch)) {
      return NextResponse.json(
        { error: 'Forbidden — chief DI review access required' },
        { status: 403 }
      );
    }

    await advanceCdiReviewAdmin(id, token.uid);
    return NextResponse.json({ success: true });
  } catch (error) {
    logError(error as Error, 'API.transferBatches.cdiReview');
    return NextResponse.json(
