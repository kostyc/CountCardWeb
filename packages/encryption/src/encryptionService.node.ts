...
/** Minimal libsodium-wrappers surface used on Expo web and React Native. */
...
type SodiumModule = {
  SodiumPlus?: SodiumPlusConstructor;
  CryptographyKey?: { from: (...args: unknown[]) => unknown };
  getBackendObject?: (
    type: 'SodiumNative' | 'LibsodiumWrappers'
  ) => { init: () => Promise<unknown> };
  default?: SodiumModule;
};
type SodiumPlusConstructor = {
  new (backend: unknown): SodiumInstance;
  auto: () => Promise<SodiumInstance>;
};
type SodiumInstance = {
  crypto_secretbox_keygen: () => Promise<{ getBuffer: () => Buffer }>;
  randombytes_buf: (size: number) => Promise<Buffer>;
  crypto_secretbox: (
...
async function getCryptographyKeyClass(): Promise<{
  from: (...args: unknown[]) => unknown;
}> {
  const exports = await getSodiumModuleExports();
  const CryptographyKey = exports.CryptographyKey;
  if (!CryptographyKey) {
    throw new Error('Failed to load sodium-plus: CryptographyKey export not found');
  }
  return CryptographyKey;
}

function toEncryptionKey(cryptoKey: { getBuffer: () => Buffer }): EncryptionKey {
  return new Uint8Array(cryptoKey.getBuffer());
}

async function toCryptographyKey(key: EncryptionKey): Promise<unknown> {
  const CryptographyKey = await getCryptographyKeyClass();
  return CryptographyKey.from(Buffer.from(key));
}
...
async function createSodiumInstance(): Promise<SodiumInstance> {
  const exports = await getSodiumModuleExports();
  const SodiumPlusClass = exports.SodiumPlus;
  if (!SodiumPlusClass) {
    throw new Error('Failed to load sodium-plus: SodiumPlus export not found');
  }

  // Metro may stub sodium-native; use WASM libsodium in browsers and React Native.
  if (isClientRuntime() && exports.getBackendObject) {
    const LibsodiumWrappersBackend = exports.getBackendObject('LibsodiumWrappers');
    const backend = await LibsodiumWrappersBackend.init();
    return new SodiumPlusClass(backend);
  }

  return SodiumPlusClass.auto();
}
...
async function getSodium(): Promise<SodiumInstance> {
  ...
  if (!sodiumInstance) {
    sodiumInstance = await createSodiumInstance();
  }
...
export async function generateKey(): Promise<EncryptionKey> {
  try {
    ...
    const cryptoKey = await sodium.crypto_secretbox_keygen();
    const key = toEncryptionKey(cryptoKey);
...
export async function generateNonce(): Promise<Uint8Array> {
  try {
    ...
    const sodium = await getSodium();
    const nonce = await sodium.randombytes_buf(NONCE_SIZE);
...
export async function generateSalt(): Promise<Uint8Array> {
  try {
    ...
    const sodium = await getSodium();
    const salt = await sodium.randombytes_buf(SALT_SIZE);
...
export async function encrypt(
  ...
  try {
    ...
    const sodium = await getSodium();
    const cryptoKey = await toCryptographyKey(key);
    ...
    const ciphertext = await sodium.crypto_secretbox(plaintextBytes, nonce, cryptoKey);

    // Encode both ciphertext and nonce as base64
    const encryptedData: EncryptionResult = {
      ciphertext: encodeBase64(new Uint8Array(ciphertext)),
      nonce: encodeBase64(new Uint8Array(nonce)),
    };
...
export async function decrypt(
  ...
  try {
    ...
    const plaintextBytes = await sodium.crypto_secretbox_open(ciphertext, nonce, cryptoKey);
...
export async function deriveKeyFromPassword(
  ...
  try {
    ...
    const derivedKey = toEncryptionKey(key);
...
export async function generateRecoveryCode(): Promise<string> {
  try {
    const randomBytes = isClientRuntime()
      ? (await getClientLibsodium()).randombytes_buf(24)
      : new Uint8Array(await (await getSodium()).randombytes_buf(24));
